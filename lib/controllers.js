(function() {
  var Journal, Segment, create_categorize_task, create_transcribe_task, dbchecker, dormouse, fs, join, json_spawn, mongoose, segment, segments_to_db_tasks, spawn, url, utils;
  ...
  segment = function(fields, files) {
    ...
    return json_spawn('python', ['pdeff/split.py'], journal.file_path, [], segments_db_tasks);
  };

  segments_to_db_tasks = function(segments) {
  ...
  create_categorize_task = function(segment) {
    ...
    return dormouse.createTask(task_info, function(r) {
      if (!r) throw new Error('Error creating dormouse task for categorization');
    });
  };
