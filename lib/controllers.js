(function() {
  ...
  exports.configure = function(server) {
    ...
    server.get('/task', function(req, res) {
      ...
      if (req.query['task_id'] != null) {
        return dmconnect.fetch_render_task_for_id(req.query['task_id'], null, function(err, task) {
          var q_assignmentId;
          if (err) console.error(err);
          if (task) context['task'] = task.html;
          context['submit'] = "http://workersandbox.mturk.com/mturk/externalSubmit";
          q_assignmentId = req.query['assignmentId'];
          context['assignmentId'] = q_assignmentId && q_assignmentId !== 'ASSIGNMENT_ID_NOT_AVAILABLE' ? q_assignmentId : false;
      ...
      if (req.form) {
        return req.form.complete(function(err, fields) {
          ...
          dormouse.answerTask(task_id, {
            ...
            if (err) {
              return console.error('Error answering categorize dormouse task', err);
            }
          });
          return res.redirect("/");
        });
      ...
      if (req.form) {
        return req.form.complete(function(err, fields) {
          ...
          dormouse.answerTask(task_id, {
            ...
            if (err) {
              return console.error('Error answering transcribe dormouse task', err);
            }
          });
          return res.redirect("/");
        });
    ...
    server.get('/complete', function(req, res) {
      if (req.query['journal_id'] != null) {
        return Journal.findById(req.query['journal_id'], function(err, journal) {
          if (journal.completed) {
            return res.render('complete.jade', {
              journal: journal
            });
          } else {
            return finalize_journal(req.query['journal_id'], function(j) {
              return res.render('complete.jade', {
                journal: j
              });
            });
    ...
    server.get('/transcribed/:journal_id', function(req, res) {
      var journal_id, text_parts;
      ...
      return Segment.find({
        ...
        return res.render('transcribed.jade', {
          'segments': text_parts
        });
      });
    });
  ...
  create_categorize_task = function(segment) {
    ...
    return dormouse.createTask(task_info, function(err, r) {
      if (err) {
        return console.error('Error creating categorize dormouse task', err);
      }
    });
  };
  ...
  create_transcribe_task = function(segment) {
    ...
    return dormouse.createTask(task_info, function(err, r) {
      if (err) {
        return console.error('Error creating transcribe dormouse task', err);
      }
    });
  };
  ...
  dbchecker = function(err, doc) {
    if (err) return console.error('Error saving to db', doc);
  };
