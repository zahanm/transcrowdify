(function() {
  var Journal, Segment, accepted_types, create_categorize_task, create_transcribe_task, dbchecker, dormouse, finalize_journal, fs, io, join, json_spawn, mongoose, notify_finalized, querystring, record_transcription, request, save_segments_to_db, spawn, split, utils,
  ...
  exports.configure = function(server) {
    ...
    server.get('/authenticate', function(req, res) {
      var api_key, code, dm_server, project_id, qs;
      ...
      return request.get(("" + dm_server + "/oauth/access_token.json?") + qs, function(err, r) {
        req.session.access_token = r['access_token'];
        console.info(req.session.access_token);
        qs = querystring.stringify({
          access_token: req.session.access_token
        });
        return request.get(("" + dm_server + "/api/v1/users/current.json?") + qs, function(err, r) {
